...
鼠标 X 的横滚灵敏度为原版鼠标转向灵敏度的 1.6 倍（常量 `ROLL_SENSITIVITY`），A/D 偏航速度约为每帧 2.5 度（常量 `KEY_YAW_STEP`）。横滚平滑系数为 0.45/游戏刻，停止飞行后的自动回正系数为 0.22/游戏刻。
...
- 激活时：
  - 鼠标 X 转为 `targetRoll` 增量（保留原版 0.15 缩放系数）。
...
功能自 `0.1.0-preview.1` 起提供，手感参数（灵敏度、偏航速度、平滑/回正系数、能量消耗）仍在预览调整阶段。
